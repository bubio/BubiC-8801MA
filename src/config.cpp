...
void initialize_config()
{
	...
	// screen
	config.window_scale_idx = 0; // Default to x1.0
...
void load_config(const _TCHAR* config_path)
{
	...
	// screen
	#ifndef ONE_BOARD_MICRO_COMPUTER
		config.window_scale_idx = MyGetPrivateProfileInt(_T("Screen"), _T("WindowScale"), config.window_scale_idx, config_path);
...
void save_config(const _TCHAR* config_path)
{
	...
	// screen
	#ifndef ONE_BOARD_MICRO_COMPUTER
		MyWritePrivateProfileInt(_T("Screen"), _T("WindowScale"), config.window_scale_idx, config_path);
...
bool process_config_state(void *f, bool loading)
{
	...
	#endif
	// Host-side audio settings are intentionally excluded from state restore.
	// Keep 2 integers in the stream for backward compatibility.
	int ignored_sound_frequency = 2;
	int ignored_sound_latency = 1;
	state_fio->StateValue(ignored_sound_frequency);
	state_fio->StateValue(ignored_sound_latency);
	return true;
}
