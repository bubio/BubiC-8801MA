...
#define EMU_LOG(fmt, ...) do { fprintf(stderr, "[EMU] " fmt "\n", ##__VA_ARGS__); fflush(stderr); } while(0)
...
static const int sound_frequency_table[5] = {
    44100,
    48000,
    55467,
    88200,
...
static const int SOUND_FREQUENCY_DEFAULT_INDEX = 2;
static const int SOUND_LATENCY_DEFAULT_INDEX = 1;
static const int SOUND_FREQUENCY_COUNT =
    (int)(sizeof(sound_frequency_table) / sizeof(sound_frequency_table[0]));
static const int SOUND_LATENCY_COUNT =
    (int)(sizeof(sound_latency_table) / sizeof(sound_latency_table[0]));

static int index_from_sound_frequency_hz(int hz)
{
  for (int i = 0; i < SOUND_FREQUENCY_COUNT; i++) {
    if (sound_frequency_table[i] == hz) {
      return i;
    }
  }
  return -1;
...
static int sanitize_sound_frequency_index(int value)
{
  // New format: index (0-4)
  if (0 <= value && value < SOUND_FREQUENCY_COUNT) {
    return value;
  }

  // Direct Hz value.
  int hz_index = index_from_sound_frequency_hz(value);
  if (hz_index >= 0) {
    return hz_index;
  }

  // Legacy 8-entry index compatibility.
  switch (value) {
  case 5: // 44100
    return 0;
  case 6: // 55467 (was 48k slot in PC-8801 build)
    return 2;
  case 7: // 96000
    return 4;
  default:
    return SOUND_FREQUENCY_DEFAULT_INDEX;
  }
}

static int sanitize_sound_latency_index(int index)
{
  return (0 <= index && index < SOUND_LATENCY_COUNT)
             ? index
             : SOUND_LATENCY_DEFAULT_INDEX;
}

static int get_sound_samples_for_index(int freq_index, int latency_index)
{
  const int rate = sound_frequency_table[freq_index];
  return (int)(rate * sound_latency_table[latency_index] + 0.5);
}

static void build_frequency_apply_order(int requested_index, int *order)
{
  bool used[SOUND_FREQUENCY_COUNT];
  memset(used, 0, sizeof(used));
  order[0] = requested_index;
  used[requested_index] = true;

  for (int pos = 1; pos < SOUND_FREQUENCY_COUNT; pos++) {
    int best_index = -1;
    ...
    for (int i = 0; i < SOUND_FREQUENCY_COUNT; i++) {
      ...
      int delta = sound_frequency_table[i] - sound_frequency_table[requested_index];
      if (delta < 0) {
        delta = -delta;
      }
...
#endif
{
  EMU_LOG("EMU constructor starting...");
  common_initialize();
#ifdef _DEBUG_LOG
  ...
  // load sound config
  config.sound_frequency = sanitize_sound_frequency_index(config.sound_frequency);
  config.sound_latency = sanitize_sound_latency_index(config.sound_latency);
  ...
  sound_samples =
      (int)(sound_rate * sound_latency_table[config.sound_latency] + 0.5);
  ...
  // initialize osd
  EMU_LOG("Creating OSD...");
#if defined(OSD_QT)
  osd = new OSD(p, csp_logger);
#else
  osd = new OSD();
#endif
  EMU_LOG("OSD created: %p", (void*)osd);
#if defined(OSD_QT)
...
#endif
  EMU_LOG("Calling osd->initialize(%d, %d)...", sound_rate, sound_samples);
  osd->initialize(sound_rate, sound_samples);
  EMU_LOG("osd->initialize() done");
  osd->emu = this;

  // initialize vm
  EMU_LOG("Creating VM...");
  osd->vm = vm = new VM(this);
  EMU_LOG("VM created: %p", (void*)vm);
  vm->update_config(); // Reflect loaded config (CPU type, boot mode, etc.)
  EMU_LOG("vm->update_config() done");
#if defined(_USE_QT)
  osd->reset_vm_node();
#endif
#ifdef USE_AUTO_KEY
  ...
  for (int i = 0; i < USE_SOUND_VOLUME; i++) {
    vm->set_sound_device_volume(i, config.sound_volume_l[i],
                                config.sound_volume_r[i]);
  }
#endif
#ifdef USE_HARD_DISK
  for (int drv = 0; drv < USE_HARD_DISK; drv++) {
    if (config.last_hard_disk_path[drv][0] != _T('\0') &&
        FILEIO::IsFileExisting(config.last_hard_disk_path[drv])) {
      vm->open_hard_disk(drv, config.last_hard_disk_path[drv]);
      my_tcscpy_s(hard_disk_status[drv].path, _MAX_PATH,
                  config.last_hard_disk_path[drv]);
    }
  }
#endif
  EMU_LOG("Calling vm->reset()...");
  vm->reset();
  EMU_LOG("vm->reset() done");

  now_suspended = false;
  EMU_LOG("EMU constructor completed");
}
...
#ifdef OSD_QT
EmuThreadClass *EMU::get_parent_handler() { return osd->get_parent_handler(); }
...
void EMU::set_host_cpus(int v) { osd->host_cpus = (v <= 0) ? 1 : v; }

int EMU::get_host_cpus() { return osd->host_cpus; }
#endif
...
const _TCHAR *EMU::device_name() { return vm->device_name(); }

double EMU::get_frame_rate() { return vm->get_frame_rate(); }
...
bool EMU::is_frame_skippable() { return vm->is_frame_skippable(); }
...
int EMU::run() {
  ...
  if (request_save_state >= 0 || request_load_state >= 0) {
    ...
    if (is_debugger_enabled(debugger_cpu_index)) {
      if (!(vm->get_device(debugger_target_id) != NULL &&
            vm->get_device(debugger_target_id)->get_debugger() != NULL)) {
        debugger_target_id = vm->get_cpu(debugger_cpu_index)->this_device_id;
      }
      DEBUGGER *cpu_debugger =
          (DEBUGGER *)vm->get_cpu(debugger_cpu_index)->get_debugger();
  ...
  // Precision timing: Synchronize VM execution with real-time.
  static const int MS_SHIFT = 16;
  static uint64_t begin_tick = 0;
  static uint64_t total_frame_time = 0;

  // Pause VM while UI is interacting to prevent glitches.
  if (osd->is_ui_interacting()) {
  ...
  if (begin_tick == 0) begin_tick = SDL_GetTicks();
  ...
  if (!config.full_speed) {
    double rate = vm->get_frame_rate();
    if (rate < 1.0) rate = 60.0;
    ms_per_frame = (uint64_t)((1000.0 * (1 << MS_SHIFT)) / rate);
  }

  auto advance_timing = [&](int frames) -> bool {
    if (config.full_speed || frames <= 0) {
      return true;
    }
    total_frame_time += ms_per_frame * (uint64_t)frames;
    // Resync if timing drift is too large (e.g. after sleep or heavy load).
    if ((int64_t)(elapsed_time - total_frame_time) > (500LL << MS_SHIFT)) {
      begin_tick = current_tick;
      total_frame_time = 0;
      return false;
    }
  ...
  // Drive audio queue every loop (common_source_project style).
  int sound_frames = 0;
  osd->update_sound(&sound_frames);
  if (sound_frames > 0) {
    ran_frames += sound_frames;
    advance_timing(sound_frames);
  }

  // Catch up VM slices when still behind schedule.
  while (config.full_speed || (int64_t)(elapsed_time - total_frame_time) >= 0) {
    if (ran_frames >= max_batches) {
      break;
    }
  ...
  if (ran_frames > 0) {
    osd->add_extra_frames(ran_frames);
    return ran_frames;
  }

  // Too early for next frame.
  // Don't sleep for the full gap here; long sleeps reduce render cadence
  // drastically when audio-driven batching advances multiple VM frames.
  // Yield briefly and let the main loop keep presenting.
  SDL_Delay(0);
  return 0;
}

bool EMU::apply_host_sound_settings() {
  if (!osd || !vm) {
    return false;
  }

  const int requested_frequency =
      sanitize_sound_frequency_index(config.sound_frequency);
  const int requested_latency =
      sanitize_sound_latency_index(config.sound_latency);

  const int prev_config_frequency = config.sound_frequency;
  const int prev_config_latency = config.sound_latency;
  const int prev_sound_frequency = sound_frequency;
  const int prev_sound_latency = sound_latency;
  const int prev_sound_rate = sound_rate;
  const int prev_sound_samples = sound_samples;

  config.sound_frequency = requested_frequency;
  config.sound_latency = requested_latency;

  int apply_order[SOUND_FREQUENCY_COUNT];
  build_frequency_apply_order(requested_frequency, apply_order);

  bool success = false;
  int applied_frequency = prev_sound_frequency;
  int applied_rate = prev_sound_rate;
  int applied_samples = prev_sound_samples;

  osd->lock_vm();

  for (int pos = 0; pos < SOUND_FREQUENCY_COUNT; pos++) {
    const int freq_index = apply_order[pos];
    const int rate = sound_frequency_table[freq_index];
    const int samples = get_sound_samples_for_index(freq_index, requested_latency);

    if (!osd->reconfigure_sound(rate, samples)) {
      continue;
    }

    sound_frequency = freq_index;
    sound_latency = requested_latency;
    sound_rate = rate;
    sound_samples = samples;
    vm->update_sound_rate(rate, samples);
#ifdef USE_SOUND_VOLUME
    for (int i = 0; i < USE_SOUND_VOLUME; i++) {
      vm->set_sound_device_volume(i, config.sound_volume_l[i],
                                  config.sound_volume_r[i]);
  ...
  if (success) {
    config.sound_frequency = applied_frequency;
    sound_frequency = applied_frequency;
    sound_latency = requested_latency;
    sound_rate = applied_rate;
    sound_samples = applied_samples;
    if (osd->is_ui_interacting()) {
      osd->set_audio_pause_for_ui(true);
    } else {
      osd->set_audio_pause_for_ui(false);
    }
  } else {
    // Rollback: keep previous working config/runtime settings.
    config.sound_frequency = prev_config_frequency;
    config.sound_latency = prev_config_latency;
    sound_frequency = prev_sound_frequency;
    sound_latency = prev_sound_latency;
    sound_rate = prev_sound_rate;
    sound_samples = prev_sound_samples;
  }
...
void EMU::notify_power_off() { vm->notify_power_off(); }
#endif

void EMU::power_off() { osd->power_off(); }
...
void EMU::lock_vm() { osd->lock_vm(); }

void EMU::unlock_vm() { osd->unlock_vm(); }

void EMU::force_unlock_vm() { osd->force_unlock_vm(); }

bool EMU::is_vm_locked() { return osd->is_vm_locked(); }
...
bool EMU::get_caps_locked() { return vm->get_caps_locked(); }

bool EMU::get_kana_locked() { return vm->get_kana_locked(); }
#endif

void EMU::key_lost_focus() { osd->key_lost_focus(); }
...
#ifdef USE_MOUSE
void EMU::enable_mouse() { osd->enable_mouse(); }

void EMU::disable_mouse() { osd->disable_mouse(); }

void EMU::toggle_mouse() { osd->toggle_mouse(); }

bool EMU::is_mouse_enabled() { return osd->is_mouse_enabled(); }
#endif
  ...
  if (!initialized) {
    ...
    if (config.keyboard_type) {
      for (int i = 0;; i++) {
        ...
        auto_key_table[auto_key_table_50on_base[i][0]] =
            auto_key_table_50on_base[i][1];
      }
    } else
#endif
      for (int i = 0;; i++) {
        ...
        auto_key_table[auto_key_table_kana_base[i][0]] =
            auto_key_table_kana_base[i][1];
      }
    ...
    for (int i = 0;; i++) {
      ...
      auto_key_table[vm_auto_key_table_base[i][0]] =
          vm_auto_key_table_base[i][1];
    }
#endif
    initialized = true;
  }
...
void EMU::set_auto_key_list(char *buf, int size) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if ((code = get_auto_key_code(code)) != 0) {
      ...
      code &= ~(0x400 |
                0x800); // don't press shift key for both alphabet and ALPHABET
...
bool is_alphabet(char code) { return (code >= 'a' && code <= 'z'); }

bool is_vowel(char code) {
  return (code == 'a' || code == 'i' || code == 'u' || code == 'e' ||
          code == 'o');
}

bool is_consonant(char code) { return (is_alphabet(code) && !is_vowel(code)); }
...
#ifdef USE_JOYSTICK
const uint32_t *EMU::get_joy_buffer() { return (const uint32_t *)joy_status; }
#endif
...
void EMU::set_host_window_size(int window_width, int window_height,
                               bool window_mode) {
  osd->set_host_window_size(window_width, window_height, window_mode);
}

void EMU::set_vm_screen_size(int screen_width, int screen_height,
                             int window_width, int window_height,
                             int window_width_aspect,
                             int window_height_aspect) {
  osd->set_vm_screen_size(screen_width, screen_height, window_width,
                          window_height, window_width_aspect,
                          window_height_aspect);
}

void EMU::set_vm_screen_lines(int lines) { osd->set_vm_screen_lines(lines); }

int EMU::get_vm_window_width() { return osd->get_vm_window_width(); }

int EMU::get_vm_window_height() { return osd->get_vm_window_height(); }
...
bool EMU::is_screen_changed() { return vm->is_screen_changed(); }
#endif
...
void EMU::reload_bitmap() { osd->reload_bitmap(); }
#endif

#ifdef OSD_WIN32
void EMU::invalidate_screen() { osd->invalidate_screen(); }

void EMU::update_screen(HDC hdc) { osd->update_screen(hdc); }
#endif

void EMU::capture_screen() { osd->capture_screen(); }

bool EMU::start_record_video(int fps) { return osd->start_record_video(fps); }

void EMU::stop_record_video() { osd->stop_record_video(); }

bool EMU::is_video_recording() { return osd->now_record_video; }

// ----------------------------------------------------------------------------
// sound
// ----------------------------------------------------------------------------

void EMU::mute_sound() { osd->mute_sound(); }

void EMU::start_record_sound() { osd->start_record_sound(); }

void EMU::stop_record_sound() { osd->stop_record_sound(); }

bool EMU::is_sound_recording() { return osd->now_record_sound; }
...
void EMU::get_video_buffer() { osd->get_video_buffer(); }

void EMU::mute_video_dev(bool l, bool r) { osd->mute_video_dev(l, r); }
#endif
...
void EMU::close_movie_file() { osd->close_movie_file(); }

void EMU::play_movie() { osd->play_movie(); }

void EMU::stop_movie() { osd->stop_movie(); }

void EMU::pause_movie() { osd->pause_movie(); }

double EMU::get_movie_frame_rate() { return osd->get_movie_frame_rate(); }

int EMU::get_movie_sound_rate() { return osd->get_movie_sound_rate(); }

void EMU::set_cur_movie_frame(int frame, bool relative) {
  osd->set_cur_movie_frame(frame, relative);
}

uint32_t EMU::get_cur_movie_frame() { return osd->get_cur_movie_frame(); }
#endif
...
int EMU::get_num_capture_devs() { return osd->get_num_capture_devs(); }
...
void EMU::close_capture_dev() { osd->close_capture_dev(); }

void EMU::show_capture_dev_filter() { osd->show_capture_dev_filter(); }

void EMU::show_capture_dev_pin() { osd->show_capture_dev_pin(); }

void EMU::show_capture_dev_source() { osd->show_capture_dev_source(); }

void EMU::set_capture_dev_channel(int ch) { osd->set_capture_dev_channel(ch); }
#endif
...
void EMU::release_bitmap(bitmap_t *bitmap) { osd->release_bitmap(bitmap); }

void EMU::create_font(font_t *font, const _TCHAR *family, int width, int height,
                      int rotate, bool bold, bool italic) {
  osd->create_font(font, family, width, height, rotate, bold, italic);
}

void EMU::release_font(font_t *font) { osd->release_font(font); }

void EMU::create_pen(pen_t *pen, int width, uint8_t r, uint8_t g, uint8_t b) {
  osd->create_pen(pen, width, r, g, b);
}

void EMU::release_pen(pen_t *pen) { osd->release_pen(pen); }
...
void EMU::draw_text_to_bitmap(bitmap_t *bitmap, font_t *font, int x, int y,
                              const char *text, uint8_t r, uint8_t g,
                              uint8_t b) {
  osd->draw_text_to_bitmap(bitmap, font, x, y, text, r, g, b);
}

void EMU::draw_line_to_bitmap(bitmap_t *bitmap, pen_t *pen, int sx, int sy,
                              int ex, int ey) {
  osd->draw_line_to_bitmap(bitmap, pen, sx, sy, ex, ey);
}

void EMU::draw_rectangle_to_bitmap(bitmap_t *bitmap, int x, int y, int width,
                                   int height, uint8_t r, uint8_t g,
                                   uint8_t b) {
  osd->draw_rectangle_to_bitmap(bitmap, x, y, width, height, r, g, b);
}

void EMU::draw_point_to_bitmap(bitmap_t *bitmap, int x, int y, uint8_t r,
                               uint8_t g, uint8_t b) {
  osd->draw_point_to_bitmap(bitmap, x, y, r, g, b);
}

void EMU::stretch_bitmap(bitmap_t *dest, int dest_x, int dest_y, int dest_width,
                         int dest_height, bitmap_t *source, int source_x,
                         int source_y, int source_width, int source_height) {
  osd->stretch_bitmap(dest, dest_x, dest_y, dest_width, dest_height, source,
...
#ifdef USE_SOCKET
SOCKET EMU::get_socket(int ch) { return osd->get_socket(ch); }

void EMU::notify_socket_connected(int ch) { osd->notify_socket_connected(ch); }
...
void EMU::disconnect_socket(int ch) { osd->disconnect_socket(ch); }

bool EMU::listen_socket(int ch) { return osd->listen_socket(ch); }

void EMU::send_socket_data_tcp(int ch) { osd->send_socket_data_tcp(ch); }

void EMU::send_socket_data_udp(int ch, uint32_t ipaddr, int port) {
  osd->send_socket_data_udp(ch, ipaddr, port);
}

void EMU::send_socket_data(int ch) { osd->send_socket_data(ch); }

void EMU::recv_socket_data(int ch) { osd->recv_socket_data(ch); }
#endif
...
#ifdef USE_MIDI
void EMU::send_to_midi(uint8_t data) { osd->send_to_midi(data); }

bool EMU::recv_from_midi(uint8_t *data) { return osd->recv_from_midi(data); }
#endif
...
void EMU::sleep(uint32_t ms) { osd->sleep(ms); }
...
void EMU::update_media() {
  ...
  for (int drv = 0; drv < USE_FLOPPY_DISK; drv++) {
    if (floppy_disk_status[drv].wait_count != 0 &&
        --floppy_disk_status[drv].wait_count == 0) {
      vm->open_floppy_disk(drv, floppy_disk_status[drv].path,
                           floppy_disk_status[drv].bank);
#if USE_FLOPPY_DISK > 1
      out_message(_T("FD%d: %s"), drv + BASE_FLOPPY_DISK_NUM,
  ...
  for (int drv = 0; drv < USE_QUICK_DISK; drv++) {
    if (quick_disk_status[drv].wait_count != 0 &&
        --quick_disk_status[drv].wait_count == 0) {
      vm->open_quick_disk(drv, quick_disk_status[drv].path);
#if USE_QUICK_DISK > 1
      out_message(_T("QD%d: %s"), drv + BASE_QUICK_DISK_NUM,
...
#ifdef USE_HARD_DISK
  for (int drv = 0; drv < USE_HARD_DISK; drv++) {
    if (hard_disk_status[drv].wait_count != 0 &&
        --hard_disk_status[drv].wait_count == 0) {
      vm->open_hard_disk(drv, hard_disk_status[drv].path);
#if USE_HARD_DISK > 1
      out_message(_T("HD%d: %s"), drv + BASE_HARD_DISK_NUM,
  ...
  for (int drv = 0; drv < USE_TAPE; drv++) {
    if (tape_status[drv].wait_count != 0 &&
        --tape_status[drv].wait_count == 0) {
  ...
  for (int drv = 0; drv < USE_COMPACT_DISC; drv++) {
    if (compact_disc_status[drv].wait_count != 0 &&
        --compact_disc_status[drv].wait_count == 0) {
      vm->open_compact_disc(drv, compact_disc_status[drv].path);
#if USE_COMPACT_DISC > 1
      out_message(_T("CD%d: %s"), drv + BASE_COMPACT_DISC_NUM,
  ...
  for (int drv = 0; drv < USE_LASER_DISC; drv++) {
    if (laser_disc_status[drv].wait_count != 0 &&
        --laser_disc_status[drv].wait_count == 0) {
      vm->open_laser_disc(drv, laser_disc_status[drv].path);
#if USE_LASER_DISC > 1
      out_message(_T("LD%d: %s"), drv + BASE_LASER_DISC_NUM,
  ...
  for (int drv = 0; drv < USE_BUBBLE; drv++) {
    if (bubble_casette_status[drv].wait_count != 0 &&
        --bubble_casette_status[drv].wait_count == 0) {
      vm->open_bubble_casette(drv, bubble_casette_status[drv].path,
                              bubble_casette_status[drv].bank);
#if USE_BUBBLE > 1
      out_message(_T("Bubble%d: %s"), drv + BASE_BUBBLE_NUM,
                  bubble_casette_status[drv].path);
#else
...
void EMU::restore_media() {
  ...
  for (int drv = 0; drv < USE_CART; drv++) {
    if (cart_status[drv].path[0] != _T('\0')) {
      if (check_file_extension(cart_status[drv].path, _T(".hex")) &&
          hex2bin(cart_status[drv].path,
                  create_local_path(_T("hex2bin.$$$")))) {
...
void EMU::open_cart(int drv, const _TCHAR *file_path) {
  if (drv < USE_CART) {
    if (check_file_extension(file_path, _T(".hex")) &&
        hex2bin(file_path, create_local_path(_T("hex2bin.$$$")))) {
      vm->open_cart(drv, create_local_path(_T("hex2bin.$$$")));
      FILEIO::RemoveFile(create_local_path(_T("hex2bin.$$$")));
    } else {
...
void EMU::open_floppy_disk(int drv, const _TCHAR *file_path, int bank) {
  if (drv < USE_FLOPPY_DISK) {
    ...
    if (check_file_extension(file_path, _T(".d88")) ||
        check_file_extension(file_path, _T(".d8e")) ||
        check_file_extension(file_path, _T(".d77")) ||
        check_file_extension(file_path, _T(".1dd"))) {
      FILEIO *fio = new FILEIO();
      if (fio->Fopen(file_path, FILEIO_READ_BINARY)) {
        try {
          ...
          while (file_offset + 0x2b0 <= file_size &&
                 d88_file[drv].bank_num < MAX_D88_BANKS) {
            ...
            MultiByteToWideChar(CP_ACP, 0, tmp, -1,
                                d88_file[drv].disk_name[d88_file[drv].bank_num],
                                18);
#else
...
uint32_t EMU::is_quick_disk_accessed() { return vm->is_quick_disk_accessed(); }
#endif

#ifdef USE_HARD_DISK
bool EMU::create_blank_hard_disk(const _TCHAR *file_path, int sector_size,
                                 int sectors, int surfaces, int cylinders) {
...
uint32_t EMU::is_hard_disk_accessed() { return vm->is_hard_disk_accessed(); }
#endif
...
uint32_t EMU::is_laser_disc_accessed() { return vm->is_laser_disc_accessed(); }
#endif
...
uint32_t EMU::get_led_status() { return vm->get_led_status(); }
#endif
...
void EMU::update_config() { vm->update_config(); }

#ifdef OSD_QT
// New APIs
void EMU::load_sound_file(int id, const _TCHAR *name, int16_t **data,
                          int *dst_size) {
    ...
    if (fio->FgetUint32() == STATE_VERSION) {
      ...
      if (process_config_state((void *)fio, true)) {
        ...
        if (reinitialize) {
          ...
          sound_rate = sound_frequency_table[config.sound_frequency];
          sound_samples =
              (int)(sound_rate * sound_latency_table[config.sound_latency] +
                    0.5);
          vm->initialize_sound(sound_rate, sound_samples);
#ifdef USE_SOUND_VOLUME
          for (int i = 0; i < USE_SOUND_VOLUME; i++) {
