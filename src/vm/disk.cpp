...
static const int secsize[8] = {128, 256, 512, 1024, 2048, 4096, 8192, 16384};
...
static const fd_format_t fd_formats[] = {
...
#elif defined(_X1) || defined(_X1TWIN) || defined(_X1TURBO) ||                 \
    defined(_X1TURBOZ)
    ...
    {MEDIA_TYPE_2HD, 77, 2, 26, 128, FM, 26, 256, MFM}, // 2HD	998KB, SORD M68
...
void DISK::open(const _TCHAR *file_path, int bank) {
  ...
  if (fio->Fopen(file_path, FILEIO_READ_BINARY)) {
    ...
    if (check_file_extension(file_path, _T(".d88")) ||
      ...
      try {
        if (nfdr0_to_d88(fio) || nfdr1_to_d88(fio)) {
          inserted = changed = true;
          my_stprintf_s(dest_path, _MAX_PATH,
                        is_d8e_image ? _T("%s.D8E") : _T("%s.D88"), file_path);
        }
      } catch (...) {
        // failed to convert the disk image
      }
    ...
    } else if (check_file_extension(file_path, _T(".hdm")) &&
               file_size.d == 77 * 2 * 8 * 1024) {
    ...
    } else if (check_file_extension(file_path, _T(".hd5")) &&
               file_size.d == 80 * 2 * 15 * 512) {
    ...
    } else if (check_file_extension(file_path, _T(".hd4")) &&
               file_size.d == 80 * 2 * 18 * 512) {
    ...
    } else if (check_file_extension(file_path, _T(".hdb")) &&
               file_size.d == 77 * 2 * 26 * 256) {
    ...
    } else if (check_file_extension(file_path, _T(".dd9")) &&
               file_size.d == 80 * 2 * 9 * 512) {
    ...
    } else if (check_file_extension(file_path, _T(".dd6")) &&
               file_size.d == 80 * 2 * 8 * 512) {
    ...
    } else if (check_file_extension(file_path, _T(".xdf")) &&
               file_size.d == 77 * 2 * 8 * 1024) {
    ...
    } else if (check_file_extension(file_path, _T(".2d")) &&
               file_size.d == 40 * 2 * 16 * 256) {
    ...
    } else if (check_file_extension(file_path, _T(".sf7")) &&
               file_size.d == 40 * 1 * 16 * 256) {
    ...
    } else if (check_file_extension(file_path, _T(".img")) &&
               file_size.d == 70 * 1 * 16 * 256) {
    ...
    if (!inserted) {
      ...
      for (int i = 0;; i++) {
        ...
        if (p->nsec2 == 0) {
          ...
          length = (uint32_t)(p->nsec * p->size +
                              (p->ncyl * p->nside - 1) * p->nsec2 * p->size2);
        }
        if (file_size.d == length) {
          ...
          try {
            //						if(solid_to_d88(fio,
            //p->type, p->ncyl, p->nside, p->nsec, p->size, p->mfm)) {
            if (solid_to_d88(fio, type, ncyl, nside, nsec, size, p->mfm,
                             p->nsec2, p->size2, p->mfm2)) {
              inserted = changed = is_solid_image = true;
            }
          } catch (...) {
            // failed to convert the disk image
  ...
  if (inserted) {
...
#if defined(_FM7) || defined(_FM8) || defined(_FM77_VARIANTS) ||               \
    defined(_FM77AV_VARIANTS)
    // FIXME: ugly patch for FM-7 Gambler Jiko Chuushin Ha, DEATH FORCE and
    // Psy-O-Blade
    if (media_type == MEDIA_TYPE_2D) {
      ...
      if (IS_VALID_TRACK(offset.d)) {
        ...
        for (int i = 0; i < sector_num.sd; i++) {
          data_size.read_2bytes_le_from(t + 14);
          if (data_size.sd == 0x100 && t[0] == 0 && t[1] == 0 && t[2] == 7 &&
              t[3] == 1) {
            /* Type 1: Sec07, +$50- "1989/09/12 ... "*/
            static const uint8_t gamblerfm_1[] = {
                0x31, 0x39, 0x38, 0x39, 0x2f, 0x30, 0x39, 0x2f, 0x31, 0x32,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x28, 0x43, 0x29, 0x47,
                0x41, 0x4d, 0x45, 0x41, 0x52, 0x54, 0x53, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x20, 0x20, 0x20, 0x59, 0x45, 0x4c, 0x4c, 0x4f,
                0x57, 0x48, 0x4f, 0x52, 0x4e, 0x00, 0x00, 0x00, 0x20, 0x20,
                0x20, 0x4b, 0x4f, 0x55, 0x44, 0x41, 0x4e, 0x53, 0x59, 0x41,
                0x20, 0x59, 0x4f, 0x55, 0x4e, 0x47, 0x2d, 0x4d, 0x41, 0x47,
                0x41, 0x5a, 0x49, 0x4e, 0x45, 0x00};
            /* Type 2: Sec07, +$30- */
            static const uint8_t gamblerfm_2[] = {
                0xb7, 0xde, 0xad, 0xdc, 0xdd, 0xcc, 0xde, 0xd7, 0xb1, 0x20,
                0xbc, 0xde, 0xba, 0xc1, 0xad, 0xb3, 0xbc, 0xdd, 0xca};
            if (memcmp((void *)(t + 0x50), gamblerfm_1, sizeof(gamblerfm_1)) ==
                0) {
              is_special_disk = SPECIAL_DISK_FM7_GAMBLER;
              break;
            }
            if (memcmp((void *)(t + 0x30), gamblerfm_2, sizeof(gamblerfm_2)) ==
                0) {
              is_special_disk = SPECIAL_DISK_FM7_GAMBLER;
              break;
            }
          } else if (data_size.sd == 0x200 && t[0] == 0 && t[1] == 0 &&
                     t[2] == 0xf7 && t[3] == 2) {
            //"DEATHFORCE/77AV" + $f7*17 + $00 + $00
            static const uint8_t deathforce[] = {
                0x44, 0x45, 0x41, 0x54, 0x48, 0x46, 0x4f, 0x52, 0x43,
                0x45, 0x2f, 0x37, 0x37, 0x41, 0x56, 0xf7, 0xf7, 0xf7,
                0xf7, 0xf7, 0xf7, 0xf7, 0xf7, 0xf7, 0xf7, 0xf7, 0xf7,
                0xf7, 0xf7, 0xf7, 0xf7, 0xf7, 0x00, 0x00};
            if (memcmp((void *)(t + 0x10), deathforce, sizeof(deathforce)) ==
                0) {
              is_special_disk = SPECIAL_DISK_FM7_DEATHFORCE;
              break;
            }
          } else if (data_size.sd == 0x100 && t[0] == 0 && t[1] == 0 &&
                     t[2] == 5 && t[3] == 1) {
            ...
            static const uint8_t xanadu2fm_d_2[] = {
                0x50, 0x72, 0x6F, 0x74, 0x65, 0x63, 0x74, 0x65, 0x64, 0x20,
                0x62, 0x79, 0x20, 0x20, 0x4E, 0x20, 0x26, 0x20, 0x41, 0x20,
                0x28, 0x53, 0x55, 0x50, 0x45, 0x52, 0x20, 0x48, 0x41, 0x43,
                0x4B, 0x45, 0x52, 0x20, 0x43, 0x4F, 0x4D, 0x42, 0x49, 0x29,
                0x43, 0x61, 0x6E, 0x20, 0x79, 0x6F, 0x75, 0x20, 0x66, 0x6F,
                0x75, 0x6E, 0x64, 0x20, 0x43, 0x48, 0x45, 0x43, 0x4B, 0x2D,
                0x52, 0x4F, 0x55, 0x54, 0x49, 0x4E, 0x45, 0x53, 0x20, 0x3F,
                0x43, 0x61, 0x6E, 0x20, 0x79, 0x6F, 0x75, 0x20, 0x63, 0x72,
                0x61, 0x63, 0x6B, 0x20, 0x74, 0x68, 0x65, 0x73, 0x65, 0x20,
                0x70, 0x72, 0x6F, 0x74, 0x65, 0x63, 0x74, 0x20, 0x3F, 0x67,
                0x6F, 0x6F, 0x64, 0x20, 0x62, 0x79, 0x65, 0x20, 0x21, 0x6F,
                0x75, 0x20,
            };
            if (memcmp((void *)(t + 0x10 + 0x60), xanadu2fm_d_1,
                       sizeof(xanadu2fm_d_1)) == 0) {
              if (memcmp((void *)(t + 0x10 + 0x70), xanadu2fm_d_2,
                         sizeof(xanadu2fm_d_2)) == 0) {
            ...
            static const uint8_t xanadu1fm_d_1[] = {
                0xFF, 0x43, 0x6F, 0x6E, 0x74, 0x69, 0x6E, 0x75, 0x65, 0x20,
                0x74, 0x68, 0x69, 0x73, 0x20, 0x70, 0x72, 0x6F, 0x67, 0x72,
                0x61, 0x6D, 0x2E, 0x0D, 0xFF, 0x00, 0x34, 0x01, 0x1C, 0xAF,
                0x6F, 0x8D, 0x01, 0x3F, 0x17, 0xFA, 0xE4, 0x27, 0xFB, 0x35,
                0x81, 0x34, 0x13, 0xA6, 0x80, 0x81, 0xFF, 0x27, 0x04, 0x8D,
                0x04, 0x20, 0xF6, 0x35, 0x93, 0x34, 0x15, 0xA7, 0x8D, 0x00,
                0xB6, 0xC6, 0x05, 0x30, 0x8D, 0x00, 0xAC, 0x17, 0x00, 0xB6,
                0x17, 0x00, 0xD6, 0x35, 0x95, 0x00, 0x00, 0x3F, 0x59, 0x41,
                0x4D, 0x41, 0x55, 0x43, 0x48, 0x49, 0x91, 0xD3,
            };
            static const uint8_t xanadu1fm_d_2[] = {
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x3F, 0x59, 0x41, 0x4D, 0x41, 0x55, 0x43,
                0x48, 0x49, 0x93, 0xD3, 0x8F, 0x90, 0x8E,
            };
            if (memcmp((void *)(t + 0x10 + 0), xanadu1fm_d_1,
                       sizeof(xanadu1fm_d_1)) == 0) {
              if (memcmp((void *)(t + 0x10 + 0xb0), xanadu1fm_d_2,
                         sizeof(xanadu1fm_d_2)) == 0) {
                is_special_disk =
                    SPECIAL_DISK_FM7_XANADU2_D; // Same issue as Xanadu2.
                break;
              }
            }
          } else if (data_size.sd == 0x100 && t[0] == 0 && t[1] == 0 &&
                     t[2] == 1 && t[3] == 1) {
            //$03 + $2D + "PSY-O-BLADE   Copyright 1988 by T&E SOFT Inc." + $B6
            //+ $FD + $05
            static const uint8_t psyoblade_ipl1[] = {
                0x03, 0x2d, 0x50, 0x53, 0x59, 0xa5, 0x4f, 0xa5, 0x42, 0x4c,
                0x41, 0x44, 0x45, 0x20, 0x20, 0x20, 0x43, 0x6f, 0x70, 0x79,
                0x72, 0x69, 0x67, 0x68, 0x74, 0x20, 0x31, 0x39, 0x38, 0x38,
                0x20, 0x62, 0x79, 0x20, 0x54, 0x26, 0x45, 0x20, 0x53, 0x4f,
                0x46, 0x54, 0x20, 0x49, 0x6e, 0x63, 0x2e, 0xb6, 0xfd, 0x05};
            // IPL Signature1
            ...
            if (memcmp((void *)(t + 0x58), psyoblade_ipl1,
                       sizeof(psyoblade_ipl1)) == 0) {
              is_special_disk = SPECIAL_DISK_FM77AV_PSYOBLADE;
              break;
            } else if (memcmp((void *)(t + 0x10), psyoblade_disk_1,
                              sizeof(psyoblade_disk_1)) == 0) {
              if (memcmp((void *)(t + 0x40), psyoblade_disk_2,
                         sizeof(psyoblade_disk_2)) == 0) {
                is_special_disk = SPECIAL_DISK_FM77AV_PSYOBLADE;
                break;
              }
            } else if (memcmp((void *)(t + 0x3e + 0x10), riglas_fm,
                              sizeof(riglas_fm)) == 0) {
            ...
            static const uint8_t taiyoufm2[] = {
                0x3d, 0x02, 0xa3, 0xd6, 0x01, 0xc7, 0x06, 0x86, 0x07, 0x00,
                0x00, 0xc7, 0x06, 0xd4, 0x01, 0x00, 0x00, 0xb4, 0x19, 0xcd,
                0x21, 0xfe, 0xc0, 0xa2, 0xda, 0x01, 0x06, 0xb9, 0x10, 0x00,
                0xbb, 0x40, 0x00, 0x8e, 0xc3, 0xbb, 0x00, 0x00, 0xfe, 0x06,
                0xd9, 0x01, 0x26, 0x80, 0xbf, 0x6c, 0x02, 0x00, 0x74, 0x03,
                0x43, 0xe2, 0xf1, 0x07, 0xc6, 0x06, 0xdb, 0x01, 0x00, 0xbb,
                0x80, 0x00, 0x80, 0x3f,
            };
    ...
    if (media_type == MEDIA_TYPE_2D) {
      ...
      if (IS_VALID_TRACK(offset.d)) {
        // check first sector
        static const uint8_t batten[] = {0xca, 0xde, 0xaf, 0xc3, 0xdd,
                                         0x20, 0xc0, 0xc7, 0xb7};
        uint8_t *t = buffer + offset.d;
#if defined(_X1TURBO) || defined(_X1TURBOZ)
//				if(strncmp((char *)(t + 0x11), "turbo ALPHA",
//11) == 0) { 					is_special_disk = SPECIAL_DISK_X1TURBO_ALPHA; 				} else
#endif
...
void DISK::close() {
  ...
  if (inserted) {
    ...
    if (/*!write_protected &&*/ !(file_size.d == orig_file_size &&
                                  get_crc32(buffer, file_size.d) ==
                                      orig_crc32)) {
      // write image
      FILEIO *fio = new FILEIO();
      ...
      // is this d88 format ?
      if (check_file_extension(dest_path, _T(".d88")) ||
          check_file_extension(dest_path, _T(".d8e")) ||
          check_file_extension(dest_path, _T(".d77")) ||
          check_file_extension(dest_path, _T(".1dd"))) {
      ...
      if ((FILEIO::IsFileExisting(dest_path) &&
           FILEIO::IsFileProtected(dest_path)) ||
          !fio->Fopen(dest_path, FILEIO_WRITE_BINARY)) {
        if (is_d8e_image) {
          fio->Fopen(local_path(create_string(
                         _T("temporary_saved_floppy_disk_#%d.D8E"), drive_num)),
                     FILEIO_WRITE_BINARY);
        } else {
          fio->Fopen(local_path(create_string(
                         _T("temporary_saved_floppy_disk_#%d.D88"), drive_num)),
                     FILEIO_WRITE_BINARY);
        }
      }
      if (fio->IsOpened()) {
...
bool DISK::get_track_tmp(int trk, int side) {
  ...
  if (media_type == MEDIA_TYPE_144 || media_type == MEDIA_TYPE_2HD) {
    if (track_mfm) {
      if (data_size.sd == 256 && sector_num.sd == 26)
        gap3_size = 54;
      if (data_size.sd == 512 && sector_num.sd == 15)
        gap3_size = 84;
      if (data_size.sd == 1024 && sector_num.sd == 8)
        gap3_size = 116;
    } else {
      if (data_size.sd == 128 && sector_num.sd == 26)
        gap3_size = 27;
      if (data_size.sd == 256 && sector_num.sd == 15)
        gap3_size = 42;
      if (data_size.sd == 512 && sector_num.sd == 8)
        gap3_size = 58;
    }
  } else {
    if (track_mfm) {
      if (data_size.sd == 256 && sector_num.sd == 16)
        gap3_size = 51;
      if (data_size.sd == 512 && sector_num.sd == 9)
        gap3_size = 80;
      if (data_size.sd == 1024 && sector_num.sd == 5)
        gap3_size = 116;
    } else {
      if (data_size.sd == 128 && sector_num.sd == 16)
        gap3_size = 27;
      if (data_size.sd == 256 && sector_num.sd == 9)
        gap3_size = 42;
      if (data_size.sd == 512 && sector_num.sd == 5)
        gap3_size = 58;
    }
  }
  ...
  if (gap3_size == 0) {
    gap3_size = (get_track_size() - total - gap0_size - gap1_size) /
                (valid_sector_num + 1);
  }
  gap4_size = get_track_size() - total - gap0_size - gap1_size -
              gap3_size * valid_sector_num;

  if (gap3_size < 8 || gap4_size < 8) {
    gap0_size = gap1_size = gap3_size =
        (get_track_size() - total) / (2 + valid_sector_num + 1);
    gap4_size = get_track_size() - total - gap0_size - gap1_size -
                gap3_size * valid_sector_num;
  }
  if (gap3_size < 8 || gap4_size < 8) {
  ...
  t = sector;
  total = preamble_size;
  sync_position[array_length(sync_position) - 1] =
      gap0_size; // sync position in preamble
  ...
  for (int i = 0; i < sector_num.sd; i++) {
    ...
    if (data_size.sd > 0) {
      ...
      for (int j = 0; j < data_size.sd; j++) {
        ...
        crc = (uint16_t)((crc << 8) ^
                         crc_table[(uint8_t)(crc >> 8) ^ t[0x10 + j]]);
      }
      if (p < track_size)
        track[p++] = (crc >> 8) & 0xff;
      if (p < track_size)
...
void DISK::set_sector_info(uint8_t *t) {
  ...
  // t[8]: 0x00 = valid, 0x10 = valid (deleted data), 0xa0 = id crc error, 0xb0
  // = data crc error, 0xe0 = address mark missing, 0xf0 = data mark missing
  sector_mfm = (t[6] == 0);
  deleted = (t[7] != 0);
  //	if(ignore_crc()) {
...
bool DISK::get_sector_info(int trk, int side, int index, uint8_t *c, uint8_t *h,
                           uint8_t *r, uint8_t *n, bool *mfm, int *length) {
  if (media_type == MEDIA_TYPE_2D && drive_type == DRIVE_TYPE_2DD) {
    if (trk >= 0) {
      if (trk & 1) {
        return false; // unformat
...
bool DISK::get_sector_info_tmp(int trk, int side, int index, uint8_t *c,
                               uint8_t *h, uint8_t *r, uint8_t *n, bool *mfm,
                               int *length) {
  // search track
  if (trk == -1 && side == -1) {
    trk = cur_track;
...
void DISK::insert_sector(uint8_t c, uint8_t h, uint8_t r, uint8_t n,
                         bool deleted, bool data_crc_error, uint8_t fill_data,
                         int length) {
...
void DISK::trim_buffer() {
  ...
  memset(buffer, 0, sizeof(buffer));
  memcpy(buffer, tmp_buffer, min((uint32_t)sizeof(buffer), file_size.d));
}
...
int DISK::get_track_size() {
  if (track_size != 0) {
    ...
    return media_type == MEDIA_TYPE_144   ? 12500
           : media_type == MEDIA_TYPE_2HD ? 10410
           : track_mfm                    ? 6250
                                          : 3100;
  } else {
    return drive_type == DRIVE_TYPE_144   ? 12500
           : drive_type == DRIVE_TYPE_2HD ? 10410
           : drive_mfm                    ? 6250
                                          : 3100;
  }
}

double DISK::get_usec_per_track() { return 1000000.0 / (get_rpm() / 60.0); }
...
typedef struct {
  ...
  uint32_t trkptr(int trk) {
    return trkptr_[trk][0] | (trkptr_[trk][1] << 8) | (trkptr_[trk][2] << 16) |
           (trkptr_[trk][3] << 24);
  }
...
typedef struct {
  uint8_t c, h, r, n;
  uint16_t nsec() { return nsec_[0] | (nsec_[1] << 8); }
  ...
  uint8_t rsrv[5];
  uint16_t size() { return size_[0] | (size_[1] << 8); }
...
/*
        this teledisk image decoder is based on:
                ...
                LZHUF.C English version 1.0 based on Japanese version
   29-NOV-1988 LZSS coded by Haruhiko OKUMURA Adaptive Huffman Coding coded by
   Haruyasu YOSHIZAKI Edited and translated to English by Kenji RIKITAKE
                TDLZHUF.C by WTK
*/
...
typedef struct {
  ...
  uint8_t dos;
  uint8_t sides;
  uint16_t crc() { return crc_[0] | (crc_[1] << 8); }
  uint8_t crc_[2];
} td_hdr_t;
...
typedef struct {
  uint16_t crc() { return crc_[0] | (crc_[1] << 8); }
  uint8_t crc_[2];
  uint16_t len() { return len_[0] | (len_[1] << 8); }
  uint8_t len_[2];
  uint8_t ymd[3];
  uint8_t hms[3];
} td_cmt_t;
...
bool DISK::teledisk_to_d88(FILEIO *fio) {
  ...
  d88_hdr.type = ((hdr.dens & 3) == 2)  ? MEDIA_TYPE_2HD
                 : ((trkcnt >> 1) > 60) ? MEDIA_TYPE_2DD
                                        : MEDIA_TYPE_2D;
  d88_hdr.size(trkptr);
...
bool DISK::imagedisk_to_d88(FILEIO *fio) {
  ...
  while (pos < size) {
    ...
    for (int i = 0; i < sector_count; i++) {
      ...
      if (stype == 0 || stype > 8) {
        ...
        d88_sct.del =
            (stype == 3 || stype == 4 || stype == 7 || stype == 8) ? 0x10 : 0;
        d88_sct.stat = (stype == 5 || stype == 6 || stype == 7 || stype == 8)
                           ? 0xb0
                           : d88_sct.del;
...
#ifndef _ANY2D88
      emu->out_debug_log(
          _T("IMD: Track=%d Head=%d Size=%d\tC=%02X H=%02X R=%02X N=%02x\n"),
          track, head, d88_sct.size(), d88_sct.c, d88_sct.h, d88_sct.r,
          d88_sct.n);
#endif
  ...
  d88_hdr.type = (img_mode == 0)              ? MEDIA_TYPE_2HD
                 : (((trkcnt + 1) >> 1) > 60) ? MEDIA_TYPE_2DD
                                              : MEDIA_TYPE_2D;
  d88_hdr.size(trkptr);
  memcpy(buffer, &d88_hdr, sizeof(d88_hdr_t));
  return true;
...
bool DISK::cpdread_to_d88(FILEIO *fio) {
  ...
  if (track_offsets_error) {
    // I found the dsk image that the track size in table is 1100h, but the
    // actual track size is 900h, so I modified this code to search "Track-Info"
    // at the top of track information block
    int cnt = 0, tmp = 0x100;
    for (int i = 0; i < tracks * heads; i++) {
      bool found = false;
  ...
  for (int track = 0; track < tracks; track++) {
    for (int side = 0; side < heads; side++) {
      ...
      for (int j = 0; j < tr.number_of_sector; j++) {
        ...
        memcpy(&sector,
               tmp_buffer + track_offsets[(track << 1) + side] + sizeof(tr) +
                   (sizeof(sector) * j),
               sizeof(sector));
        ...
        d88_sct.stat = (d88_sct.size() == 0)              ? 0xf0
                       : (sector.fdc_status_reg1 == 0xb5) ? 0xb0
                                                          : d88_sct.del;
  ...
  d88_hdr.type = (total < (368640 + 655360) / 2)    ? MEDIA_TYPE_2D
                 : (total < (737280 + 1228800) / 2) ? MEDIA_TYPE_2DD
                                                    : MEDIA_TYPE_2HD;
  d88_hdr.size(trkptr);
  memcpy(buffer, &d88_hdr, sizeof(d88_hdr_t));
  return true;
...
typedef struct {
  ...
  {
    return byHeadSize[0] | (byHeadSize[1] << 8) | (byHeadSize[2] << 16) |
           (byHeadSize[3] << 24);
  }
  BYTE byHeadSize[4];
...
typedef struct {
  ...
  {
    return byTrackHead[trk][0] | (byTrackHead[trk][1] << 8) |
           (byTrackHead[trk][2] << 16) | (byTrackHead[trk][3] << 24);
  }
  BYTE byTrackHead[164][4];
  DWORD dwAddInfo() /* 追加情報ヘッダのアドレス */
  {
    return byAddInfo[0] | (byAddInfo[1] << 8) | (byAddInfo[2] << 16) |
           (byAddInfo[3] << 24);
  }
  BYTE byAddInfo[4];
...
typedef struct {
  ...
  BYTE byRetry;  /* RetryDataなし(0)あり(1-) */
  DWORD dwDataLen() {
    return byDataLen[0] | (byDataLen[1] << 8) | (byDataLen[2] << 16) |
           (byDataLen[3] << 24);
  }
  BYTE byDataLen[4];
  BYTE byPDA;              /* PDA                      */
...
bool DISK::solid_to_d88(FILEIO *fio, int type, int ncyl, int nside, int nsec,
                        int size, bool mfm) {
  return solid_to_d88(fio, type, ncyl, nside, nsec, size, mfm, 0, 0, false);
}

bool DISK::solid_to_d88(FILEIO *fio, int type, int ncyl, int nside, int nsec,
                        int size, bool mfm, int nsec2, int size2, bool mfm2) {
  int n = 0, t = 0;
...
bool DISK::process_state(FILEIO *state_fio, bool loading) {
  ...
  if (loading) {
    ...
    state_fio->FputInt32_LE(sector ? (int)((LONG_PTR)sector - (LONG_PTR)buffer)
                                   : -1);
    state_fio->FputInt32_LE(
        unstable ? (int)((LONG_PTR)unstable - (LONG_PTR)buffer) : -1);
  }
