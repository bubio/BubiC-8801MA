...
static const uint8_t intr_mask2_table[8] = {
    (uint8_t)~7, (uint8_t)~3, (uint8_t)~5, (uint8_t)~1,
    (uint8_t)~6, (uint8_t)~2, (uint8_t)~4, (uint8_t)~0};
...
// #endif
  ...
  if (fio->Fopen(create_local_path(_T("80SRCG.ROM")), FILEIO_READ_BINARY)) {
    ...
    fio->Fclose();
  } else if (fio->Fopen(create_local_path(_T("HIRAFONT.ROM")),
                        FILEIO_READ_BINARY)) {
    fio->Fread(hiragana, 0x200, 1);
    fio->Fclose();
  } else {
  ...
  for (int i = 1; i <= 8; i++) {
    if (fio->Fopen(create_local_path(create_string(_T("E%d.ROM"), i)),
                   FILEIO_READ_BINARY)) {
      long length = fio->FileLength();
      fio->Fread(n88erom[i], 0x2000, 1);
      fio->Fclose();
      if (length < 0x2000)
        memset(&n88erom[i][length], 0xff, 0x2000 - length);
      n88erom_loaded |= (1 << i);
    }
  }
  ...
  if (config.option_switch & OPTION_SWITCH_16BIT) {
    if (fio->Fopen(create_local_path(_T("PC-8801-16_Z80.ROM")),
                   FILEIO_READ_BINARY)) {
  ...
  // Check essential ROMs
  bool rom_missing = false;
  _TCHAR missing_list[1024] = _T("");
#if defined(PC8001_VARIANT)
  if (n80rom[0] == 0xff) {
    rom_missing = true;
    my_tcscat_s(missing_list, 1024, _T("N80.ROM "));
  }
#else
  if (n88rom[0] == 0xff) {
    rom_missing = true;
    my_tcscat_s(missing_list, 1024, _T("PC88.ROM (or N88.ROM) "));
  }
  if (n80rom[0] == 0xff) {
    rom_missing = true;
    my_tcscat_s(missing_list, 1024, _T("N80.ROM "));
...
#endif
  if (rom_missing) {
    emu->out_message(_T("Essential ROM files are missing:\n%s\n\nSearch ")
                     _T("path:\n1. %s\n2. %s"),
                     missing_list, get_initial_current_path(),
...
void PC88::write_io8(uint32_t addr, uint32_t data) {
  ...
  this->out_debug_log(_T("%06x\tOUT8\t%02x,%02x\n"), d_cpu->get_pc(), addr,
                      data);
#endif
#if defined(_PC8001) || defined(_PC8001MK2) || defined(_PC8801) ||             \
    defined(_PC8801MK2)
...
#ifdef NIPPY_PATCH
  // dirty patch for NIPPY
  if (addr == 0x31 && data == 0x3f && d_cpu->get_pc() == 0xaa4f &&
      nippy_patch) {
  ...
  case 0x32:
    ...
    if (mod & 0x10) {
      // XM8 version 1.10
      //			if(config.boot_mode == MODE_PC88_V1H ||
      // config.boot_mode == MODE_PC88_V2 || config.boot_mode == MODE_PC88_V2CD)
      //{
      update_tvram_memmap();
      f000_m1_wait_clocks = get_m1_wait(true);
      //			}
  ...
  case 0x40:
    ...
    if (mod & 0x40) {
      ...
      if (mouse_phase == 0) {
        // latch position
        mouse_lx = -((mouse_dx > 127)    ? 127
                     : (mouse_dx < -127) ? -127
                                         : mouse_dx);
        mouse_ly = -((mouse_dy > 127)    ? 127
                     : (mouse_dy < -127) ? -127
                                         : mouse_dy);
        mouse_dx = mouse_dy = 0;
      }
    ...
    if (config.dipswitch & DIPSWITCH_CMDSING) {
      ...
      d_pcm->write_signal(SIG_PCM1BIT_SIGNAL,
                          ((beep_on && beep_signal) || sing_signal) ? 1 : 0, 1);
    }
#endif
    break;
#ifdef SUPPORT_PC88_OPN1
  ...
  case 0x80:
    if (d_pio_16bit != NULL) {
      d_pio_16bit->write_signal(SIG_I8255_PORT_C, 0x04,
                                0x04); // STB_B(PC2): H->L->H
  ...
  case 0x90:
    if (config.boot_mode == MODE_PC88_V2CD && cdbios_loaded && (mod & 0x01)) {
      if (d_scsi_host) {
        ...
        //				d_scsi_host->write_signal(SIG_SCSI_SEL,
        // data, 1);
      }
    }
    break;
  ...
  case 0x98:
    if (config.boot_mode == MODE_PC88_V2CD && cdbios_loaded) {
      ...
      case 4:
        ...
        register_event(this, EVENT_FADE_IN, 100, true,
                       &cdda_register_id); // 100ms
        if (d_scsi_cdrom) {
          d_scsi_cdrom->set_volume((int)(cdda_volume = 0.0));
        }
        break;
      case 5:
        ...
        register_event(this, EVENT_FADE_IN, 1500, true,
                       &cdda_register_id); // 1500ms
        if (d_scsi_cdrom) {
          d_scsi_cdrom->set_volume((int)(cdda_volume = 0.0));
        }
        break;
      case 6:
        ...
        register_event(this, EVENT_FADE_OUT, 100, true,
                       &cdda_register_id); // 100ms
        if (d_scsi_cdrom) {
          d_scsi_cdrom->set_volume((int)(cdda_volume = 100.0));
        }
      ...
      case 7:
        ...
        register_event(this, EVENT_FADE_OUT, 5000, true,
                       &cdda_register_id); // 5000ms
        if (d_scsi_cdrom) {
          d_scsi_cdrom->set_volume((int)(cdda_volume = 100.0));
        }
        break;
...
#endif
{
...
#if defined(_PC8001MK2) || defined(_PC8001SR)
  case 0x30:
    return (config.boot_mode == MODE_PC80_N ? 0 : 1) |
           (config.boot_mode == MODE_PC80_V2 ? 0 : 2) | 0xfc;
  case 0x31:
  ...
  case 0x30:
    ...
    // 80x20 (XM8 version 1.00)
    return (config.boot_mode == MODE_PC88_N ? 0 : 1) | 0xc2; // 80x25
  case 0x31:
    // XM8 version 1.10
    return (config.boot_mode == MODE_PC88_V2 ||
                    config.boot_mode == MODE_PC88_V2CD
                ? 0
                : 0x80) |
           (config.boot_mode == MODE_PC88_V1S || config.boot_mode == MODE_PC88_N
                ? 0
                : 0x40) |
           0x39;
    //		return (config.boot_mode == MODE_PC88_V2 || config.boot_mode ==
    // MODE_PC88_V2CD ? 0 : 0x80) | (config.boot_mode == MODE_PC88_V1S ||
    // config.boot_mode == MODE_PC88_N ? 0 : 0x40);
  case 0x32:
    return port[0x32];
#endif
  case 0x40:
    // XM8 version 1.10
    //		return (crtc.vblank ? 0x20 : 0) | (d_rtc->read_signal(0) ? 0x10
    //: 0) | (usart_dcd ? 4 : 0) | (hireso ? 0 : 2) | 0xc1;
    return (crtc.vblank ? 0x20 : 0) | (d_rtc->read_signal(0) ? 0x10 : 0) |
           (usart_dcd ? 4 : 0) | (hireso ? 0 : 2) |
           (d_prn->read_signal(SIG_PRINTER_BUSY) ? 1 : 0) | 0xc0;
#ifdef SUPPORT_PC88_OPN1
  case 0x44:
    if (d_opn1 != NULL) {
  ...
  case 0x80:
    if (d_pio_16bit != NULL) {
      d_pio_16bit->write_signal(SIG_I8255_PORT_C, 0x40,
                                0x40); // ACK_A(PC6): H->L->H
      val = porta_16bit;
  ...
  case 0x90:
    if (config.boot_mode == MODE_PC88_V2CD && cdbios_loaded) {
      ...
      // do not show BSY,MSG,CxD,IxD when SEL=1 (信長の野望 武将風雲録)
      if (port[0x90] & 0x01) {
...
#if defined(PC8801SR_VARIANT)
  if ((addr & 0xf000) == 0xf000 &&
      (config.boot_mode == MODE_PC88_V1H || config.boot_mode == MODE_PC88_V2 ||
       config.boot_mode == MODE_PC88_V2CD)) {
...
void PC88::update_timing() {
  ...
  double frames_per_sec =
      (hireso ? 24860.0 * 56.423 / 56.5 : 15980.0) / (double)lines_per_frame;
  //	double frames_per_sec = (hireso ? 24860.0 * 56.424 / 56.5 : 15980.0) /
...
void PC88::update_config() {
  ...
  uint32_t clocks = 3993624;
#ifdef SUPPORT_PC88_HIGH_CLOCK
  if (config.cpu_type == 0 || config.cpu_type == 2) {
    clocks = 7987248;
  }
#endif
  ((VM*)vm)->pc88event->set_context_cpu_clock(d_cpu, clocks);
...
int PC88::get_gvram_wait(bool read) {
  ...
  if (Port31_GRAPH) {
    ...
    if (cpu_clock_low) {
      ...
      if (config.boot_mode == MODE_PC80_V1 || config.boot_mode == MODE_PC80_N) {
#else
      if (config.boot_mode == MODE_PC88_V1S ||
          config.boot_mode == MODE_PC88_N) {
#endif
#endif
        // V1S or N
...
void PC88::event_vline(int v, int clock) {
  ...
  if (v == 0) {
    if (crtc.status & 0x10) {
      ...
      busreq_clocks =
          (int)((double)(dmac.ch[2].count.sd + 1) *
                    (cpu_clock_low ? 5.95 : 10.58) / (double)disp_line +
                0.5);
      //			busreq_clocks =
      //(int)((double)(dmac.ch[2].count.sd + 1) * (cpu_clock_low ? 7.0 : 16.0) /
      //(double)disp_line + 0.5);
    }
    crtc.start();
...
void PC88::draw_screen() {
  ...
  for (int y = 0; y < ymax; y++) {
    if (palette_line_changed[y]) {
      for (int i = 0; i < 9; i++) {
        ...
        palette_digital_text_pc[i] =
            RGBA_COLOR(pex[palette_line_digital[y][i].r],
                       pex[palette_line_digital[y][i].g],
                       pex[palette_line_digital[y][i].b], 255);
        palette_analog_text_pc[i] = RGBA_COLOR(
            pex[palette_line_analog[y][i].r], pex[palette_line_analog[y][i].g],
            pex[palette_line_analog[y][i].b], 255);
        palette_digital_graph_pc[i] =
            RGBA_COLOR(pex[palette_line_digital[y][i].r],
                       pex[palette_line_digital[y][i].g],
                       pex[palette_line_digital[y][i].b], 0);
        palette_analog_graph_pc[i] = RGBA_COLOR(
            pex[palette_line_analog[y][i].r], pex[palette_line_analog[y][i].g],
            pex[palette_line_analog[y][i].b], 0);
      }
      // set back color to black if cg screen is off in color mode
      if (!disp_color_graph) {
        palette_digital_text_pc[0] = palette_analog_text_pc[0] =
            palette_digital_graph_pc[0] = palette_analog_graph_pc[0] = 0;
      }
    ...
    if (ymax == 200) {
      for (int i = 0; i < 9; i++) {
        palette_line_digital_text_pc[2 * y][i] =
            palette_line_digital_text_pc[2 * y + 1][i] =
                palette_digital_text_pc[i];
        palette_line_analog_graph_pc[2 * y][i] =
            palette_line_analog_graph_pc[2 * y + 1][i] =
                palette_analog_graph_pc[i];
#if defined(PC8801_VARIANT)
        palette_line_analog_text_pc[2 * y][i] =
            palette_line_analog_text_pc[2 * y + 1][i] =
                palette_analog_text_pc[i];
        palette_line_digital_graph_pc[2 * y][i] =
            palette_line_digital_graph_pc[2 * y + 1][i] =
                palette_digital_graph_pc[i];
#endif
      }
    } else {
  ...
  if (PortB4_VAB_DISP) {
    ...
    for (int y = 0; y < 400; y++) {
      ...
      //			if(Port31_HCOLOR) {
      //				pal_t = palette_line_digital_text_pc
      //[yy]; 				pal_g = palette_line_analog_graph_pc
      //[yy]; 			} else
      if (Port32_PMODE) {
...
void PC88::draw_text() {
  if (emu->now_waiting_in_debugger) {
    ...
    // crtc.expand_buffer
    for (int cy = 0, ofs = 0; cy < crtc.height;
         cy++, ofs += 80 + crtc.attrib.num * 2) {
    ...
    if (crtc.mode & 4) {
      ...
      for (int cy = 0, ofs = 0; cy < crtc.height;
        ...
        for (int cx = 0; cx < crtc.width; cx += 2) {
          ...
          crtc.attrib.expand[cy][cx] = crtc.attrib.expand[cy][cx + 1] =
              crtc.attrib.data;
        }
      }
    } else {
    ...
    if (crtc.cursor.x < 80 && crtc.cursor.y < 200) {
      if ((crtc.cursor.type & 1) && crtc.blink.cursor) {
        ...
        crtc.attrib.expand[crtc.cursor.y][crtc.cursor.x] ^=
            ctype[crtc.cursor.type + 1];
      }
    }
  }
  if (crtc.status & 0x88) {
...
#endif
  //	for(int cy = 0, ytop = 0; cy < 64 && ytop < 400; cy++, ytop +=
  // char_height) {
  for (int cy = 0, ytop = 0; cy < crtc.height && ytop < 400;
       cy++, ytop += char_height) {
    for (int x = 0, cx = 0; cx < crtc.width; x += 8, cx++) {
      ...
      //			uint8_t color = !(Port30_COLOR && (attrib & 8))
      //? 7 : (attrib & 0xe0) ? (attrib >> 5) : 8;
      ...
      for (int l = 0, y = ytop; l < char_height / 2 && y < 400; l++, y += 2) {
        ...
        if (Port30_40) {
          // from ePC-8801MA改
          static const uint8_t wct[16] = {0x00, 0x03, 0x0c, 0x0f, 0x30, 0x33,
...
void PC88::draw_320x200_attrib_graph() {
  if (!Port31_GRAPH || (Port53_G0DS && Port53_G1DS && Port53_G2DS &&
                        Port53_G3DS && Port53_G4DS && Port53_G5DS)) {
    memset(graph, 0, sizeof(graph));
    return;
  }
  uint8_t *gvram_b0 = Port53_G0DS ? gvram_null : (gvram + 0x0000);
...
void PC88::notify_intr_ei() { update_intr(); }
...
void pc88_crtc_t::update_blink() {
  ...
  blink.cursor =
      (blink.counter <= blink.rate / 4) ||
      (blink.rate / 2 <= blink.counter && blink.counter <= 3 * blink.rate / 4);
}
...
void pc88_crtc_t::expand_buffer(bool hireso, bool line400) {
  ...
  for (int cy = 0, ytop = 0, ofs = 0; cy < height && ytop < 200;
       cy++, ytop += char_height_tmp, ofs += 80 + attrib.num * 2) {
...
void pc88_crtc_t::set_attrib(uint8_t code) {
  if (mode & 2) {
    ...
    if (code & 8) {
      ...
      // set attrib
      attrib.data =
          (attrib.data & 0xf0) | ((code >> 2) & 0x0d) | ((code << 1) & 2);
...
bool PC88::process_state(FILEIO *state_fio, bool loading) {
  ...
  state_fio->StateArray(&crtc.attrib.expand[0][0], sizeof(crtc.attrib.expand),
                        1);
