...
//	ステータス
//
uint32_t DiskIO::GetStatus(uint32_t a) { return status; }
...
void DiskIO::CmdReadFile() {
  ...
  case idlephase:
    ...
    if (file->Fopen(create_absolute_path(char_to_tchar((char *)filename)),
                    FILEIO_READ_BINARY)) {
...
void DiskIO::initialize() { file = new FILEIO(); }

void DiskIO::release() { delete file; }

void DiskIO::reset() { this->Reset(0, 0); }
