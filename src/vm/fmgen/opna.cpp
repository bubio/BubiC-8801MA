...
//	このオプションを有効にすると ADPCM メモリへのアクセス(特に 8bit
// モード)が 	多少軽くなるかも
//
...
//	割り込み信号の取得
bool OPNBase::ReadIRQ() { return interrupt; }
...
//	割り込み信号の設定
void OPNBase::Intr(bool value) { interrupt = value; }
...
uint OPNA::GetReg(uint addr) {
  ...
  if (addr == 0x108) {
    ...
    if ((control1 & 0x60) == 0x20) {
      ...
      //			LOG2("Rd [0x%.6x:%.2x] ", memaddr, adpcmreadbuf
      //>> 8);
    }
    //		LOG0("%.2x\n");
    return data;
...
bool OPNA::LoadRhythmSample(const _TCHAR *path) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    // Parse RIFF WAV
    uint8 head[12];
    if (file.Fread(head, 12, 1) != 1 || memcmp(head, "RIFF", 4) || memcmp(head + 8, "WAVE", 4)) {
      break;
    }
    ...
    while (true) {
      uint8 chunkid[4];
      uint32 chunksize;
      if (file.Fread(chunkid, 4, 1) != 1) break;
      if (file.Fread(&chunksize, 4, 1) != 1) break;
      ...
      if (memcmp(chunkid, "fmt ", 4) == 0) {
        ...
        if (chunksize < 16 || file.Fread(&fmt, 16, 1) != 1) break;
        wav_tag = fmt.tag;
        wav_nch = fmt.nch;
        wav_rate = fmt.rate;
        if (chunksize > 16) file.Fseek(chunksize - 16, FILEIO_SEEK_CUR);
      } else if (memcmp(chunkid, "data", 4) == 0) {
        wav_fsize = chunksize;
        break; // Found data!
      } else {
        file.Fseek(chunksize, FILEIO_SEEK_CUR);
      }
    }

    if (wav_fsize == 0 || wav_tag != 1 || wav_nch != 1) {
      break;
    }

    uint32 sample_count = wav_fsize / 2;
    if (sample_count >= 0x100000) {
      break;
    }

    delete[] rhythm[i].sample;
    rhythm[i].sample = new int16[sample_count];
    if (!rhythm[i].sample) {
      break;
    }

    file.Fread(rhythm[i].sample, wav_fsize, 1);

    rhythm[i].rate = wav_rate;
    rhythm[i].step = rhythm[i].rate * 1024 / rate;
    rhythm[i].pos = rhythm[i].size = sample_count * 1024;
  }
  if (i != 6) {
    for (int j = 0; j < 6; j++) {
      delete[] rhythm[j].sample;
      rhythm[j].sample = 0;
    }
    return false;
  }
...
void OPNA::RhythmMix(Sample *buffer, uint count) {
  if ((rhythmtvol_l < 128 || rhythmtvol_r < 128) && rhythm[0].sample &&
      (rhythmkey & 0x3f)) {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if ((rhythmkey & (1 << i)) && r.level < 128) {
        int db_l =
            Limit(rhythmtl + rhythmtvol_l + r.level + r.volume_l, 127, -31);
        int db_r =
            Limit(rhythmtl + rhythmtvol_r + r.level + r.volume_r, 127, -31);
...
//	初期化
//
bool OPNB::Init(uint c, uint r, bool ipflag, uint8 *_adpcma, int _adpcma_size,
...
//	拡張ステータスを読みこむ
//
uint OPNB::ReadStatusEx() { return (status & stmask) >> 8; }
  ...
  if ((adpcmatvol_l < 128 || adpcmatvol_r < 128) && (adpcmakey & 0x3f)) {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if ((adpcmakey & (1 << i)) && r.level < 128) {
        ...
        int db_l =
            Limit(adpcmatl + adpcmatvol_l + r.level + r.volume_l, 127, -31);
        int db_r =
            Limit(adpcmatl + adpcmatvol_r + r.level + r.volume_r, 127, -31);
        int vol_l = tltable[FM_TLPOS + (db_l << (FM_TLBITS - 7))] >> 4;
        int vol_r = tltable[FM_TLPOS + (db_r << (FM_TLBITS - 7))] >> 4;
