...
#if !defined(_ANY2D88) && !defined(DISABLE_ZLIB)
#if defined(__has_include)
#if __has_include(<zlib.h>) || __has_include("zlib-1.2.11/zlib.h")
#define USE_ZLIB
#endif
#else
#define USE_ZLIB
...
#endif
#elif defined(SDL_BYTEORDER) &&                                                \
    (defined(SDL_LIL_ENDIAN) || defined(SDL_BIG_ENDIAN))
...
typedef int errno_t;
#endif
//	errno_t DLL_PREFIX my_tfopen_s(FILE** pFile, const _TCHAR *filename,
// const _TCHAR *mode);
...
// win32 api
#ifndef _WIN32
BOOL MyWritePrivateProfileString(LPCTSTR lpAppName, LPCTSTR lpKeyName,
...
UINT MyGetPrivateProfileInt(LPCTSTR lpAppName, LPCTSTR lpKeyName, INT nDefault,
                            LPCTSTR lpFileName);
...
#define RGB_COLOR(r, g, b)                                                     \
  (((uint32_t)(r) << 16) | ((uint32_t)(g) << 8) | ((uint32_t)(b) << 0))
