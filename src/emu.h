...
public:
  ...
  media_status_t binary_file_status[USE_BINARY_FILE];
#endif
#ifdef USE_BUBBLE
...
public:
  ...
  VM_TEMPLATE *get_vm() { return vm; }
  OSD *get_osd() { return osd; }
#ifdef OSD_QT
  // qt dependent
  ...
  void start_auto_key();
  void stop_auto_key();
  bool is_auto_key_running() { return (auto_key_phase != 0); }
  FIFO *get_auto_key_buffer() { return auto_key_buffer; }
#endif
  ...
  void set_host_window_size(int window_width, int window_height,
                            bool window_mode);
  ...
  // sound
  int get_sound_rate() { return sound_rate; }
  int get_sound_samples() { return sound_samples; }
  bool apply_host_sound_settings();
  void mute_sound();
...
#endif
#ifdef USE_HARD_DISK
  bool create_blank_hard_disk(const _TCHAR *file_path, int sector_size,
...
#ifdef OSD_QT
  // New APIs
  void load_sound_file(int id, const _TCHAR *name, int16_t **data,
                       int *dst_size);
