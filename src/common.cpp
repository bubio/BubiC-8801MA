...
void DLL_PREFIX common_initialize() {
  ...
  // initialize application data path and create directories
...
errno_t DLL_PREFIX my_tcscat_s(_TCHAR *strDestination, size_t numberOfElements,
                               const _TCHAR *strSource) {
  _tcscat(strDestination, strSource);
  return 0;
}

errno_t DLL_PREFIX my_strcpy_s(char *strDestination, size_t numberOfElements,
                               const char *strSource) {
  strcpy(strDestination, strSource);
  return 0;
}

errno_t DLL_PREFIX my_tcscpy_s(_TCHAR *strDestination, size_t numberOfElements,
                               const _TCHAR *strSource) {
  _tcscpy(strDestination, strSource);
  return 0;
}

errno_t DLL_PREFIX my_tcscpy_s(_TCHAR *strDestination,
                               const _TCHAR *strSource) {
  _tcscpy(strDestination, strSource);
  return 0;
}

errno_t DLL_PREFIX my_strncpy_s(char *strDestination, size_t numberOfElements,
                                const char *strSource, size_t count) {
  strncpy(strDestination, strSource, count);
  return 0;
}

errno_t DLL_PREFIX my_tcsncpy_s(_TCHAR *strDestination, size_t numberOfElements,
                                const _TCHAR *strSource, size_t count) {
  _tcsncpy(strDestination, strSource, count);
  return 0;
}

char *DLL_PREFIX my_strtok_s(char *strToken, const char *strDelimit,
                             char **context) {
  return strtok(strToken, strDelimit);
}

_TCHAR *DLL_PREFIX my_tcstok_s(_TCHAR *strToken, const char *strDelimit,
                               _TCHAR **context) {
  return _tcstok(strToken, strDelimit);
}

int DLL_PREFIX my_sprintf_s(char *buffer, size_t sizeOfBuffer,
                            const char *format, ...) {
  va_list ap;
  va_start(ap, format);
...
int DLL_PREFIX my_swprintf_s(wchar_t *buffer, size_t sizeOfBuffer,
...
int DLL_PREFIX my_stprintf_s(_TCHAR *buffer, size_t sizeOfBuffer,
                             const _TCHAR *format, ...) {
  va_list ap;
  va_start(ap, format);
...
int DLL_PREFIX my_vsprintf_s(char *buffer, size_t numberOfElements,
                             const char *format, va_list argptr) {
  return vsprintf(buffer, format, argptr);
}

int DLL_PREFIX my_vstprintf_s(_TCHAR *buffer, size_t numberOfElements,
...
void DLL_PREFIX *my_memcpy(void *dst, void *src, size_t len) {
  ...
  if (((s_align & 0x0f) == 0x0) ||
      ((d_align & 0x0f) == 0x0)) { // Align to 128bit
    uint32_t b128[4];
    len2 = len1 >> 4;
    while (len2 > 0) {
      for (i = 0; i < 4; i++)
  ...
  if (((s_align & 0x07) == 0x0) ||
      ((d_align & 0x07) == 0x0)) { // Align to 64bit
    uint32_t b64[2];
    len2 = len1 >> 3;
    while (len2 > 0) {
      for (i = 0; i < 2; i++)
...
#ifndef _WIN32
BOOL DLL_PREFIX MyWritePrivateProfileString(LPCTSTR lpAppName,
  ...
  if (fio_i->Fopen(lpFileName, FILEIO_READ_ASCII)) {
    ...
    fio_i->Fclose();
    if (result) {
      if (!(FILEIO::RemoveFile(lpFileName) &&
            FILEIO::RenameFile(tmp_path, lpFileName))) {
        result = FALSE;
      }
    }
  } else {
...
DWORD DLL_PREFIX MyGetPrivateProfileString(LPCTSTR lpAppName, LPCTSTR lpKeyName,
                                           LPCTSTR lpDefault,
...
UINT DLL_PREFIX MyGetPrivateProfileInt(LPCTSTR lpAppName, LPCTSTR lpKeyName,
...
uint8_t DLL_PREFIX A_OF_COLOR(scrntype_t c) { return 0; }
...
scrntype_t DLL_PREFIX RGBA_COLOR(uint32_t r, uint32_t g, uint32_t b,
                                 uint32_t a) {
  return RGB_COLOR(r, g, b);
}
...
const _TCHAR *DLL_PREFIX get_application_path() {
  ...
  if (!initialized) {
    ...
    if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, 0, szPath))) {
      my_stprintf_s(app_path, _MAX_PATH, _T("%s\\BubiC-8801MA\\"), szPath);
    } else {
      // Fallback
      GetModuleFileName(NULL, app_path, _MAX_PATH);
      PathRemoveFileSpec(app_path);
      my_tcscat_s(app_path, _MAX_PATH, _T("\\"));
    }
    // Create directory if it doesn't exist
    SHCreateDirectoryEx(NULL, app_path, NULL);

#elif defined(__APPLE__)
    const char *home = getenv("HOME");
    if (home) {
      my_stprintf_s(app_path, _MAX_PATH,
                    "%s/Library/Application Support/BubiC-8801MA/", home);
      std::filesystem::create_directories(app_path);
    } else {
      my_tcscpy_s(app_path, _MAX_PATH, "./");
    }

#elif defined(__linux__)
    const char *home = getenv("HOME");
    if (home) {
      my_stprintf_s(app_path, _MAX_PATH, "%s/.local/share/BubiC-8801MA/",
                    home);
      std::filesystem::create_directories(app_path);
    } else {
      my_tcscpy_s(app_path, _MAX_PATH, "./");
...
#else
    (void)getcwd(current_path, _MAX_PATH);
#endif
...
const _TCHAR *DLL_PREFIX create_local_path(const _TCHAR *format, ...) {
  ...
  // 1. Check if file exists in the initial current directory
  my_stprintf_s(file_path[output_index], _MAX_PATH, _T("%s%s"),
                get_initial_current_path(), file_name);
  FILE *f = _tfopen(file_path[output_index], _T("rb"));
  if (f) {
    fclose(f);
    return (const _TCHAR *)file_path[output_index];
  }

  // 2. Fallback to application path (for writing or if not found in current)
  my_stprintf_s(file_path[output_index], _MAX_PATH, _T("%s%s"),
                get_application_path(), file_name);
  return (const _TCHAR *)file_path[output_index];
}

void DLL_PREFIX create_local_path(_TCHAR *file_path, int length,
                                  const _TCHAR *format, ...) {
  _TCHAR file_name[_MAX_PATH];
  va_list ap;
  ...
  my_vstprintf_s(file_name, _MAX_PATH, format, ap);
  va_end(ap);
  my_stprintf_s(file_path, length, _T("%s%s"), get_application_path(),
                file_name);
}
...
void DLL_PREFIX create_absolute_path(_TCHAR *file_path, int length,
                                     const _TCHAR *format, ...) {
  _TCHAR file_name[_MAX_PATH];
  va_list ap;
  ...
  if (is_absolute_path(file_name)) {
    ...
    my_stprintf_s(file_path, length, _T("%s%s"), get_initial_current_path(),
                  file_name);
  }
}
...
#ifdef _WIN32
  if (_tcslen(file_path) > 2 &&
      ((file_path[0] >= _T('A') && file_path[0] <= _T('Z')) ||
       (file_path[0] >= _T('a') && file_path[0] <= _T('z'))) &&
      file_path[1] == _T(':')) {
    return true;
  }
#endif
  return (_tcslen(file_path) > 1 &&
          (file_path[0] == _T('/') || file_path[0] == _T('\\')));
}
...
const _TCHAR *DLL_PREFIX create_date_file_path(const _TCHAR *extension) {
  ...
  return create_local_path(
      _T("%d-%0.2d-%0.2d_%0.2d-%0.2d-%0.2d.%s"), cur_time.year, cur_time.month,
      cur_time.day, cur_time.hour, cur_time.minute, cur_time.second, extension);
}

void DLL_PREFIX create_date_file_path(_TCHAR *file_path, int length,
                                      const _TCHAR *extension) {
...
const _TCHAR *DLL_PREFIX create_date_file_name(const _TCHAR *extension) {
  ...
  get_host_time(&cur_time);
  my_stprintf_s(file_name[output_index], _MAX_PATH,
                _T("%d-%0.2d-%0.2d_%0.2d-%0.2d-%0.2d.%s"), cur_time.year,
                cur_time.month, cur_time.day, cur_time.hour, cur_time.minute,
                cur_time.second, extension);
  return (const _TCHAR *)file_name[output_index];
}

void DLL_PREFIX create_date_file_name(_TCHAR *file_path, int length,
                                      const _TCHAR *extension) {
  my_tcscpy_s(file_path, length, create_date_file_name(extension));
}

bool DLL_PREFIX check_file_extension(const _TCHAR *file_path,
                                     const _TCHAR *ext) {
#if defined(_USE_QT)
  ...
  if (s_fpath.length() < s_ext.length())
    return false;
  pos = s_fpath.rfind(s_ext.c_str(), s_fpath.length());
  if ((pos != (int)std::string::npos) &&
      (pos >= ((int)s_fpath.length() - (int)s_ext.length())))
    return true;
  return false;
#else
  ...
  return (nam_len >= ext_len &&
          _tcsncicmp(&file_path[nam_len - ext_len], ext, ext_len) == 0);
#endif
}

const _TCHAR *DLL_PREFIX
get_file_path_without_extensiton(const _TCHAR *file_path) {
...
void DLL_PREFIX get_long_full_path_name(const _TCHAR *src, _TCHAR *dst,
                                        size_t dst_len) {
#ifdef _WIN32
...
int32_t DLL_PREFIX muldiv_s32(int32_t nNumber, int32_t nNumerator,
...
uint32_t DLL_PREFIX muldiv_u32(uint32_t nNumber, uint32_t nNumerator,
...
const _TCHAR *DLL_PREFIX get_value_or_symbol(symbol_t *first_symbol,
                                             const _TCHAR *format,
                                             uint32_t addr) {
  static _TCHAR name[8][1024];
  static unsigned int table_index = 0;
...
const _TCHAR *DLL_PREFIX get_value_and_symbol(symbol_t *first_symbol,
                                              const _TCHAR *format,
                                              uint32_t addr) {
  static _TCHAR name[8][1024];
  static unsigned int table_index = 0;
